
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <inttypes.h>
#include <stddef.h>
#include <stdbool.h>
#include <time.h>

#define num_thread 512
#define warp 32


__global__ void result(float *y,
                       int count
                           )
{
    for(int i=0;i<count;i++){
        printf("i=%i, %f\n",i, y[i]);
    }
    
}




__global__ void compute(int    M,
                        int    N,
                        int    K,
                        float *A,
                        float *x,
                        float *y)
{
    const int mat_x = blockIdx.y*blockDim.y + threadIdx.y;
    const int mat_y = blockIdx.x*blockDim.x + threadIdx.x;
    const int mat_idx = mat_x*M + mat_y;

    if(mat_x<N && mat_y<M){
        for(int idx=0; idx<K; idx++){
            y[mat_idx] = y[mat_idx] + A[mat_y + idx*M] * x[mat_x*K + idx];
        }
    }

    // if(threadIdx.x==0 && threadIdx.y==0){
    //     printf("blk-x=%u, blk-y=%u \n", blockIdx.x, blockIdx.y);
    // }
}




__global__ void setmemoryf(float *A,
                           float value, 
                           int   count)
{
    const int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if(idx<count){
        A[idx] = value;
    }
    // if(threadIdx.x==0 && blockIdx.x==0){
    // for(int i=0;i<count;i++){
    //     printf("i=%i, %f\n",i, A[i]);
    // }}
}




int main(int argc, char *argv[]){

    // hardware parameter for A3000
    // const int num_SM = 32;
    // const int num_warp = 32;
    // 4 warp schedulers.

    // input parameter
for(int test = 128; test<10000; test=test+32){

    int M = 32;
    int N = 32;
    int K = 32;
M=test;
N=test;
K=test;

    //size_t num_thread = 512;
    dim3 threadblock(warp, warp);
    dim3 blockgrid( (unsigned int)(ceilf((float)M / (float)warp) ), (unsigned int)(ceilf((float)N / (float)warp) ) );
    
    float *A, *x, *y;
    // allocate memory on the device
    cudaMalloc( &A, M*K * sizeof(float));
    cudaMalloc( &x, K*N * sizeof(float));
    cudaMalloc( &y, M*N * sizeof(float));

    // initialize variable value
    //cudaMemset is only for integer!!!
    //cudaMemset( A, 1, M*N);
    //cudaMemset( x, 3, N);
    //cudaMemset( y, 5, M);
    setmemoryf<<<(unsigned int)(ceilf((float)M*K / (float)num_thread) ), num_thread, 0, 0>>>(A, 1.0f, M*K);
    setmemoryf<<<(unsigned int)(ceilf((float)K*N / (float)num_thread) ), num_thread, 0, 0>>>(x, 3.0f, K*N);
    setmemoryf<<<(unsigned int)(ceilf((float)M*N / (float)num_thread) ), num_thread, 0, 0>>>(y, 0.0f, M*N);
    cudaDeviceSynchronize();

    // compute and timing
    struct timespec start, end;
    timespec_get(&start, TIME_UTC);

    int repeat = 1;
    for(int i=0;i<repeat;i++){
        compute<<<blockgrid, threadblock, 0, 0>>>(M, N, K, A, x, y);   //only for special case M=K=N
        cudaDeviceSynchronize();
    }

    timespec_get(&end, TIME_UTC);
    time_t d_sec  = end.tv_sec  - start.tv_sec;
    long   d_nsec = end.tv_nsec - start.tv_nsec;
    double total_time = (double)d_sec + (double)d_nsec/1000000000.0;
    //printf("time=%f\n", total_time);
    double perf = 1.0e-9 * ((double)M*(double)N*(double)K*(double)2) / total_time * (double)repeat;
    printf("test= %i , time(sec)= %f ,memory throughput = %f GFlop/s\n", test, total_time, perf);

    // show result
    //result<<<1,1>>>(y,M*N);


    // Free the memory on the device
    cudaFree(A);
    cudaFree(x);
    cudaFree(y);
}
    return 0;
}